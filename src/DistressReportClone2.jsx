...
export default function DistressReportClone2() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      // Native form submit avoids XHR CORS "Network Error" while still downloading ZIP.
      const form = e.currentTarget;
      setSuccessMessage("Submitting to Clone 2 endpoint. ZIP download should start automatically.");
      form.submit();
    } catch (err) {
      let detail = null;
      ...
      setErrorMessage(detail || "Failed to generate Clone 2 report. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="App">
      <header className="app-header">
        <div className="app-header-left">
          ...
          <span className="app-header-title">Distress Report Generator (Clone 2)</span>
        </div>
      ...
      <iframe
        title="clone2-download-frame"
        name="clone2-download-frame"
        style={{ display: "none" }}
      />
        ...
        <div className="w-full max-w-xl">
          <div className="mb-6 text-center">
            <h1 className="text-2xl font-semibold tracking-tight text-slate-50 sm:text-3xl">
              Distress Report Generator (Clone 2)
            </h1>
            <p className="mt-2 text-sm text-slate-300">
              Upload a KML file and date range. The request is sent to Clone 2 and the
              returned ZIP is downloaded.
            </p>
          </div>
          <div className="rounded-3xl border border-slate-700/70 bg-slate-900/60 p-6 shadow-2xl shadow-cyan-500/10 backdrop-blur-2xl">
            <form
              ...
              className="space-y-5"
              action="https://distress-clone2.up.railway.app/screen"
              method="post"
              encType="multipart/form-data"
              target="clone2-download-frame"
            >
