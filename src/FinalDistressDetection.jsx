...
import { runRoadAnomalyScreening } from "./ApiService";

export default function FinalDistressDetection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const result = await runRoadAnomalyScreening({
        file,
        startDate,
        endDate,
        lengthKm,
      ...
      const filename =
        result.filename ||
        `road_anomaly_results_${startDate}_to_${endDate}.zip`;

      setZipBlob(result.blob);
      setDownloadName(filename);
      setSuccessMessage("Road anomaly screening finished. ZIP download will start automatically.");
      downloadBlob(result.blob, filename);
    } catch (err) {
      const detail = await parseAxiosErrorDetail(err);
      setErrorMessage(
        detail ||
          "Failed to run road anomaly screening. Please check your input and try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDownload = () => {
    if (!zipBlob) return;
    const filename =
      downloadName || `road_anomaly_results_${startDate || "start"}_to_${endDate || "end"}.zip`;
    ...
    <div className="App">
      <header className="app-header">
        <div className="app-header-left">
          ...
          <span className="app-header-title">Final Distress Detection</span>
        </div>
        ...
        <div className="w-full max-w-xl">
          <div className="mb-6 text-center">
            {/* <h1 className="text-2xl font-semibold tracking-tight text-slate-50 sm:text-3xl">
              Run road anomaly screening
            </h1> */}
            <p className="mt-2 text-sm text-slate-300">
              Upload a merged KML file and choose the date range & corridor info to generate a ZIP output.
            </p>
          </div>
